...
/*   Updated: 2022/03/13 19:05:21 by rpapagna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	convert_s(t_mods modifiers, va_list ap)
{
	...
	if (ft_strlen(s2) && modifiers.fl.minus == 1)
	{
		if (modifiers.width <= (int)ft_strlen(s2))
			...
			nbyte += left_justify(modifiers, s2, ft_strlen(s2));
		return ((ft_pipewrench("-s", &s2) + nbyte) - 1);
	}
	if (modifiers.width > (int)ft_strlen(s2))
		nbyte += right_justify(modifiers, s2, ft_strlen(s2));
	else
		nbyte += (int)write(1, s2, ft_strlen(s2));
	return ((ft_pipewrench("-s", &s2) + nbyte) - 1);
}
